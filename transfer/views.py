...
from .forms import TransferForm,ReservationForm,ReservationQueryForm,BireyselFaturaForm,KurumsalFaturaForm,LoginForm,BlogForm,WebAboutForm,CarForm,GalleryForm
...
def reservation(request,transfer_id,car_id):
    ...
    if form.is_valid():
        ...
        if return_date == "on":
            if request.POST.get('date_of_return') == None and request.POST.get('time_of_return') == None and request.POST.get('flightnumber_of_return') == "-":
                messages.info(request,_("Lütfen 'Dönüş Ekle' kısmını doldurunuz!") )
                return render(request,"reservation.html",context)
